      ...
      <section className="workspace">
        ...
        <section className="data-card">
          ...
          {activeSection === "customers" ? <CustomersView customers={customers.filter((customer) => !search.trim() || `${customer.name} ${customer.phone}`.toLowerCase().includes(search.toLowerCase()))} selectedPhone={selectedCustomerPhone} onSelect={setSelectedCustomerPhone} onEditOrder={openEdit} /> : <>
            {activeTab === "orders" && <OrdersTable orders={visibleOrders} onEdit={openEdit} onCancel={cancelOrder} />}
...
function exportOrderPdf(order: Order) {
  ...
  printWindow.document.write(`<!DOCTYPE html><html><head><meta charset="utf-8" />
    <title>Phiếu đơn ${order.code}</title>
    <style>
...
function OrdersTable({ orders, onEdit, onCancel }: { orders: Order[]; onEdit: (id: number) => void; onCancel: (id: number) => void }) {
